...
RepeatTime = 2;
FrameNum = 100;
SubCarrierNum = 48; % every OFDM frame has 52 subcarriers
LowPaddingNum = 5; % padding number of low frequency
HighPaddingNum = 6; % padding number of high frequency
...
for j = 1: 1
    try
        ...
        %%% % Error handling
        %%% % -----------------------------------------------------------
        [waveformASC,DataSerial]= TransmitterTopNew(N,M,Ncp,RepeatTime,FrameNum,LowPaddingNum,HighPaddingNum,SubCarrierNum,Pilot_pos,WORD_LENGTH,FRACTION_LENGTH);
        ...
        while(1)
            ...
            if(SymbolSynEnable)
                ...
                if SymbolDetected == RepeatTime
                    ...
                    PayLoadLength = LengthReceivedSeq + L1 + L2 + 1 - i;
                    plot_enable = 1;
                end
                if Synchronized
                    if(FrameCounter <= RepeatTime-1)
                        ...
                        if PayLoadLength-(FrameCounter-RepeatTime+1)*(N+Ncp) >= 0 && FrameCounter - RepeatTime < FrameNum
                            ...
                            SymbolOnSpecificSubcarrier = OFDMSymbolWithoutPadding(47,:);
                            SymbolOnallSubcarrier = reshape(OFDMSymbolWithoutPadding,1,size(OFDMSymbolWithoutPadding,1)*size(OFDMSymbolWithoutPadding,2));
                            break;
                        end
