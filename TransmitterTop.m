function [Output2DAC,DataSerial,FrameNum]= TransmitterTop(N,M,Ncp,RepeatTime,NUM,NBITS,LowPaddingNum,HighPaddingNum,SubCarrierNum,Pilot_pos)
%clear all;
%close all;
%clc;
...
%NBITS = 8; %Data width of parallel output
%NUM = 1000; %Byte number
FrameNum = ceil(NUM*NBITS/M/SubCarrierNum);
...
[DataParallel,~] = prbs_gen(POLY_LENGTH, POLY_TAP, NBITS, NUM);
DataSerial = reshape(DataParallel,1,NBITS*NUM);


%% Adding pad to the data
if mod(NUM*NBITS/M,SubCarrierNum) == 0
	DataSerialAddPadding = DataSerial;
else
	DataSerialAddPadding = [DataSerial,zeros(1,(SubCarrierNum-mod(NUM*NBITS/M,SubCarrierNum))*M)];
end

%% Symbol Mapping
DataSymbol = qammod(DataSerialAddPadding',2^M,'gray','InputType','bit','UnitAveragePower',true);
...
DataSymbolQuan = complex(DataSymbolRealQuan,DataSymbolImagQuan);
...
Output2DAC = [ShortTrainingSeqOutput,LongTrainingSeqOutput,DataSymbolAddingCPQuan];