...
%% Global Parameter
N= 128;
M = 4; %16QAM modulation scheme
Ncp = 32;
RepeatTime = 2;
FrameNum = 200; % ofdm symbol number of each package transmitted
TotalFrameNum = 200; % total ofdm symbol number you want to get
...
%[waveformASC,DataSymbolQuanReshape,DataSerialOri]= TransmitterTopNew(N,M,Ncp,RepeatTime,FrameNum,LowPaddingNum,HighPaddingNum,SubCarrierNum,Pilot_pos,WORD_LENGTH,FRACTION_LENGTH);
while(1)
    try
        ...
        waveformASC_NEG = scope.QueryASCII_ListOfDoubles('FORM ASC;:CHAN2:DATA?', samplesCount); % samplesCount is the maximum allowed samples to read
                ...
                if Synchronized
                    if(FrameCounter <= RepeatTime-1)
                        ...
                        if SymbolCounter == N+Ncp
                            ...
                            % SFO correction
                            OneOFDMSymbolAfterSFOCompensation = OneOFDMSymbolAfterChannelEstimation .* exp(-1j*S_L .* [1:N/2-1]);
